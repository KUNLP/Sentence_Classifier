...
#
#
#     print(len(example.merge["sentence"]),len(word_idxs), max([span[0] for span in example.dependency["sentence"]] + [span[1] for span in example.dependency["sentence"]]))
...
#
#         # word_idxs: [[cls] 토큰의 위치]+[<WORD> 토큰의 위치]
#         word_idxs = [x+len(paddings) for x in word_idxs]
#
#     # """
#     # mean pooling
#     not_word_list = []
#     word_masks = [1] * len(word_idxs[1:]) + [0] * (max_sentence_length-len(word_idxs[1:]))
...
def convert_example_to_features(example, max_seq_length, is_training, max_sentence_length, language):
    ...
    for case in example.merge.keys():
        for merge_idx in example.merge[case]:
            chunk_start_idxs.append(len(all_doc_tokens[case]))
            for word_idx in merge_idx:
                ## word를 토큰화하여 나온 첫번째 token의 위치
                # orig_to_tok_index[case].append(len(tok_to_orig_index[case]))
                sub_tokens = tokenizer.tokenize(example.doc_tokens[case][word_idx])
                for sub_token in sub_tokens:
                    # token 저장
                    all_doc_tokens[case].append(sub_token)
                    ## token에 대한 word의 위치
                    ...
                    ##token_to_orig_map[case][len(tok_to_orig_index[case]) - 1] = len(orig_to_tok_index[case]) - 1
                    # token_to_orig_map[case].append(len(orig_to_tok_index[case]) - 1)
            chunk_end_idxs.append(len(all_doc_tokens[case]))
    ...
    if language == "KorSciBERT":
        ...
        input_ids += [tokenizer.convert_tokens_to_ids(token) for token in all_doc_tokens["sentence"]]
    chunk_start_idxs = [i + 1 for i in chunk_start_idxs]
    chunk_end_idxs = [i + 1 for i in chunk_end_idxs]
    ...
    if tokenizer.padding_side == "right":
        ...
        # chunk_start_idxs: [토큰에 대한 chunk의 시작 위치]
        # chunk_end_idxs: [토큰에 대한 chunk의 끝 위치 + 1]
        chunk_start_idxs = [x + len(paddings) for x in chunk_start_idxs]
        chunk_end_idxs = [x + len(paddings) for x in chunk_end_idxs]

    # """
    # mean pooling
    not_word_list = []
    word_masks = [1] * len(chunk_start_idxs) + [0] * (max_sentence_length - len(chunk_start_idxs))
    for chunk_start_idx, chunk_end_idx in zip(chunk_start_idxs, chunk_end_idxs):
        not_word_idxs = [0] * len(input_ids)
        for j in range(chunk_start_idx, chunk_end_idx):
            not_word_idxs[j] = 1 / (chunk_end_idx - chunk_start_idx)
        not_word_list.append(not_word_idxs)
    not_word_list = not_word_list + [[0] * len(input_ids)] * (
            max_sentence_length - len(not_word_list))
    ...
    # dependency  = [[tail, head, dependency], [], ...]
    if example.dependency["sentence"] == [[]]:
        example.dependency["sentence"] = [[max_sentence_length - 1, max_sentence_length - 1, 0] for _ in
                                          range(0, max_sentence_length)]
    else:
        example.dependency["sentence"] = example.dependency["sentence"] + [
            [max_sentence_length - 1, max_sentence_length - 1, 0] for i in
            range(0, abs(max_sentence_length - len(example.dependency["sentence"])))]
...
def convert_examples_to_features(
    ...
    if return_dataset == "pt":
        ...
        all_word_masks = torch.tensor([f.word_masks for f in features], dtype=torch.long)

        if not is_training:
            dataset = TensorDataset(
                all_input_ids,
                all_attention_masks, all_token_type_ids, all_position_ids,
                # all_cls_index, all_p_mask, all_example_indices, all_feature_index,
                all_dependency,
                all_not_word_list, all_word_masks,
            )
        else:
...
class CLASSIFIERExample(object):
    def __init__(
        ...
        label_dict = {'문제 정의': 0, '가설 설정': 1, '기술 정의': 2, '제안 방법': 3, '대상 데이터': 4, '데이터처리': 5, '이론/모형': 6, '성능/효과': 7,
                      '후속연구': 8, '기타': 9}
        ...
        if ([words for words in self.parsing if words[2][0] != words[2][1]] == []):
            if len(self.doc_tokens["sentence"]) == 1:
                merge_index.append([0])
                merge_word.append([0, 0, depend2idx["VP-None"]])
            else:
                merge_word.append([])
        else:
            for words in self.parsing:
                if words[2][0] != words[2][1]:
                    ...
                    if [w1, w2] not in check_merge_word:
                        ...
                        if (words[2][1] in ['SBJ', 'CNJ', 'OBJ']) and (
                                now_dep in ['CMP', 'MOD', 'AJT', 'None', "UNDEF"]):
