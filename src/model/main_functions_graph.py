...
from src.functions.metric import get_sklearn_score

from sklearn.metrics import confusion_matrix
from functools import partial

from dgl import DGLGraph
def get_graph(dependency, device="cpu"):
    # [문장내에서 word단위로 잘랐을때, 피지배소 위치, 지배소 위치, 구문 구조 태그 index]
    ...
    for depend in dependency:
        depend = to_list(depend)

        g = DGLGraph()
        depend = [span for span in depend if span[2] != 0]
        head = [span[1] for span in depend] + [span[0] for span in depend]
        tail = [span[0] for span in depend] + [span[1] for span in depend]
        tag = [span[2] for span in depend] + [span[2] + 63 for span in depend]  # _inv
        ...
        g.add_edges(tail, head)

        g = g.to(device)
        g.ndata.update({'id': node_ids.to(device)})
        g.edata.update({'type': edge_type.to(device), 'norm': edge_norm.to(device)})
        graphs.append(g)
...
def train(args, model, tokenizer, logger):
    ...
    for epoch in mb:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            inputs_list2 = ['word_masks', 'word_idxs']
            for m, input in enumerate(inputs_list2): inputs[input] = batch[-(m+1)]

            inputs["g"] = get_graph(dependency=batch[-3], device=args.device)
...
def evaluate(args, model, tokenizer, logger, epoch_idx="", tr_loss=1):
    ...
    for batch in progress_bar(eval_dataloader):
        ...
        if args.coarse_add: pred_coarse_logits = torch.cat([pred_coarse_logits, outputs[1]], dim=0)
    ...
    idx2label = {0: '문제 정의', 1: '가설 설정', 2: '기술 정의', 3: '제안 방법', 4: '대상 데이터', 5: '데이터처리', 6: '이론/모형', 7: '성능/효과',
                 8: '후속연구'}  # , 9: '기타'}
    ...
    if args.coarse_add: coarse_results = get_sklearn_score(pred_coarse_labels, gold_coarse_labels, idx2coarse_label)
    ...
    with open(output_eval_file, out_file_type, encoding='utf-8') as f:
        ...
        f.write("micro_accuracy: " + str(round((sum(confusion_list[0]) / all_cnt), 8)) + "\n")
        print("micro_accuracy: " + str(round((sum(confusion_list[0]) / all_cnt), 8)))
